                    ...
                    <tbody>
                        @foreach ($evaluations as $evaluation)
                            <tr>
                                ...
                                <td>{{ $evaluation->status ? 'Active' : 'Closed' }}</td>
                                <td>
                                    <form method="POST" action="{!! route('evaluations.evaluation.destroy', $evaluation->id) !!}" accept-charset="UTF-8">
                                        @method('DELETE')
                                        {{ csrf_field() }}
                                        <div class="btn-group btn-group-xs pull-right" role="group">
                                            {{-- @permission('evaluation_unit_view') --}}
                                            <a href="{{ route('evaluations.evaluation.edit', $evaluation->id) }}"
                                                class="btn btn-success" title=" Evaluation by Organization Unit">
                                                {{ __('employee.Units') }}
                                            </a>
                                            {{-- @endpermission --}}
                                            {{-- @permission('evaluation_show') --}}
                                            <a href="{{ route('evaluations.evaluation.show', $evaluation->id) }}"
                                                class="btn btn-primary" title="Show Evaluation">
                                                <span class="fa fa-eye" aria-hidden="true"></span>
                                            </a>
                                            {{-- @endpermission --}}
                                            {{-- @permission('evaluation_edit') --}}
                                            <a href="{{ route('evaluations.evaluation.edit', $evaluation->id) }}"
                                                class="btn btn-warning" title="Edit Evaluation">
