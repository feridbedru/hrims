...
<div class="row">
    <div class="form-group col-md-6 {{ $errors->has('bank') ? 'has-error' : '' }}">
        <label for="bank" class="col-md-12 control-label">{{ __('setting.Banks') }} <span
        ...
        <div class="col-md-12">
            <select class="form-control" id="bank" name="bank" required="true">
                <option value="" style="display: none;"
                    ...
                    selected>{{ __('employee.Select bank') }}</option>
    ...
    <div class="form-group col-md-6 {{ $errors->has('account_type') ? 'has-error' : '' }}">
        <label for="account_type" class="col-md-12 control-label">{{ __('setting.Bank Account Type') }} <span
                class="text-danger">*</span></label>
        <div class="col-md-12">
            <select class="form-control" id="account_type" name="account_type" required="true">
                <option value="" style="display: none;"
                    ...
                    disabled selected>{{ __('employee.Enter bank account type here') }}</option>
...
<div class="row">
    <div class="form-group col-md-6 {{ $errors->has('account_number') ? 'has-error' : '' }}">
        <label for="account_number" class="col-md-12 control-label">{{ __('employee.Account Number') }} <span
                class="text-danger">*</span></label>
        <div class="col-md-12">
            <input class="form-control" name="account_number" type="number" oninput="processs(this)" id="account_number"
                value="{{ old('account_number', optional($employeeBankAccount)->account_number) }}" required="true"
                placeholder="{{ __('employee.Enter account number here') }}">
        </div>
    </div>
