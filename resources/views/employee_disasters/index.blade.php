@extends('layouts.employee')
@section('pagetitle')
    Employee Disasters
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item active">Employee Disasters</li>
@endsection
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">Employee Disasters List</h3>
            <div class="card-tools">
                <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i>
                </button>
            </div>
        </div>

        <div class="card-body">  
        @if(count($employeeDisasters) == 0)
                <h4 class="text-center">No Employee Disasters Available.</h4>
        @else
        <table class="table table-striped ">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Employee</th>
                            <th>Occured On</th>
                            <th>Disaster Cause</th>
                            <th>Disaster Severity</th>
                            <th>Description</th>
                            <th>Status</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                    @foreach($employeeDisasters as $employeeDisaster)
                        <tr>
                                <td>{{ $loop->iteration }}</td>
                            <td>{{ $employeeDisaster->en_name }}</td>
                            <td>{{ $employeeDisaster->occured_on }}</td>
                            <td>{{ $employeeDisaster->cause }}</td>
                            <td>{{ $employeeDisaster->severity }}</td>
                            <td>{{ $employeeDisaster->description }}</td>
                            <td>{{ $employeeDisaster->status }}</td>

                            <td>
                                <form method="POST" action="{!! route('employee_disasters.employee_disaster.destroy', $employeeDisaster->id) !!}" accept-charset="UTF-8">
                                <input name="_method" value="DELETE" type="hidden">
                                {{ csrf_field() }}
                                    <div class="btn-group btn-group-xs pull-right" role="group">
                                        <a href="{{ route('employee_disasters.employee_disaster.show', $employeeDisaster->id ) }}" class="btn btn-primary" title="Show Employee Disaster">
                                            <span class="fa fa-eye" aria-hidden="true"></span>
                                        </a>
                                        <a href="{{ route('employee_disasters.employee_disaster.edit', $employeeDisaster->id ) }}" class="btn btn-warning" title="Edit Employee Disaster">
                                            <span class="fa fa-edit text-white" aria-hidden="true"></span>
                                        </a>

                                        <button type="submit" class="btn btn-danger" title="Delete Employee Disaster" onclick="return confirm(&quot;Click Ok to delete Employee Disaster.&quot;)">
                                            <span class="fa fa-trash" aria-hidden="true"></span>
                                        </button>
                                    </div>
                                </form>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
            {!! $employeeDisasters->render() !!}
        @endif
        </div>
    </div>
    <div class="btn-group btn-group-sm pull-right" role="group">
        <a href="{{ route('employee_disasters.employee_disaster.create') }}" class="btn btn-success" title="Create New Employee Disaster">
            <span class="fa fa-plus" aria-hidden="true"> Add New</span>
        </a>
    </div>
@endsection