...
@endsection
@section('pagetitle')
{{(__('setting.CommitmentFor'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item active">{{(__('setting.CommitmentFor'))}}</li>
@endsection
@section('stylesheets')
    ...
    <script type="text/javascript">
        function deleteConfirmation(id) {
            swal.fire({
                title: "Delete?",
                type: 'question',
                text: "Are you sure you want to delete this Commitment For?",
                type: "warning",
                showCancelButton: !0,
                ...
                if (e.value === true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ url('settings/commitment_fors/delete') }}/" + id,
                        data: {
                            _token: CSRF_TOKEN
                        },
                        dataType: 'JSON',
...
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">{{(__('setting.CommitmentForsList'))}}</h3>
        </div>
        ...
        <div class="card-body">
            ...
            @else
                <table class="table table-striped" id="commitment_for_table">
                    <thead>
                        <tr>
...
@section('javascripts')
    ...
    <script>
        $(document).ready(function() {
            var table = $('#commitment_for_table').DataTable({
                paging: false,
                info: false,
                colReorder: true,
            ...
            $("#commitment_for_table_filter").addClass("d-inline float-right");
            $("<hr>").insertBefore("#commitment_for_table");
        });
