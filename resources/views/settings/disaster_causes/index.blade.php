...
@endsection
@section('pagetitle')
{{(__('setting.DisasterCause'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item active">{{(__('setting.DisasterCause'))}}</li>
@endsection
@section('stylesheets')
    ...
    <script type="text/javascript">
        function deleteConfirmation(id) {
            swal.fire({
                title: "Delete?",
                type: 'question',
                text: "Are you sure you want to delete this disaster cause?",
                type: "warning",
                showCancelButton: !0,
                ...
                if (e.value === true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ url('settings/disaster_causes/delete') }}/" + id,
                        data: {
                            _token: CSRF_TOKEN
                        },
                        dataType: 'JSON',
...
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">{{(__('setting.DisasterCauseList'))}}</h3>
        </div>

        <div class="card-body">
            @permission('disasterCause_list')
            @if (count($disasterCauses) == 0)
                <h4 class="text-center">{{(__('setting.NoDisasterCausesAvailable'))}}.</h4>
            @else
                <table class="table table-striped" id="disaster_cause_table">
                    <thead>
                        <tr>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    <a href="{{ route('disaster_causes.disaster_cause.edit', $disasterCause->id) }}"
                                        class="btn btn-warning mr-4" title="Edit Disaster Cause">
                                        <span class="fa fa-edit text-white" aria-hidden="true"></span>
                                    </a>
                                    @endpermission
                                    @permission('setting_DisasterCause_Delete')
                                    <button class="btn btn-danger remove-data"
                                        onclick="deleteConfirmation({{ $disasterCause->id }})">
                ...
                {{ $disasterCauses->links() }}
                </div>
            @endif
            @endpermission
        </div>
    </div>
    @permission('setting_DisasterCause_AddNew')
...
@section('javascripts')
    ...
    <script>
        $(document).ready(function() {
            var table = $('#disaster_cause_table').DataTable({
                paging: false,
                info: false,
                colReorder: true,
            ...
            $("#disaster_cause_table_filter").addClass("d-inline float-right");
