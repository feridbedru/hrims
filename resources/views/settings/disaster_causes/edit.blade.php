@extends('layouts.app')
@section('pagetitle')
{{(__('employee.Edit Disaster Cause'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('disaster_causes.disaster_cause.index') }}">{{(__('employee.Disaster Cause'))}}</a></li>
...
@section('javascripts')
    ...
    <script>
        $(function() {
            $('#edit_disaster_cause_form').validate({
                rules: {
                    name: {
                        required: true,
                        minlength: 2,
