@extends('layouts.app')
@section('pagetitle')
{{(__('setting.NewEducationalField'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('educational_fields.educational_field.index') }}">{{(__('setting.EducationalField'))}}</a>
    </li>
...
@section('javascripts')
    ...
    <script>
        $(function() {
            $('#create_educational_field_form').validate({
                rules: {
                    name: {
                        required: true,
                        minlength: 2,
