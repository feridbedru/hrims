@extends('layouts.app')
@section('pagetitle')
{{(__('employee.Edit Job Type'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('job_types.job_type.index') }}">{{(__('employee.Job Type'))}}</a></li>
    <li class="breadcrumb-item active">{{(__('setting.edit'))}}</li>
@endsection
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title mb-1">{{(__('employee.Edit Job Type'))}}</h3>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('job_types.job_type.update', $jobType->id) }}" id="edit_job_type_form"
                ...
                <div class="form-group">
                    <div class="col-md-offset-2 col-md-12 text-center">
                        ...
                        <a href="{{ route('job_types.job_type.index') }}" class="btn btn-warning"
