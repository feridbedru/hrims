...
@endsection
@section('pagetitle')
{{(__('setting.Templates'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item active">{{(__('setting.Templates'))}}</li>
@endsection
@section('stylesheets')
...
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">{{(__('setting.TemplatesList'))}}</h3>
        </div>

        <div class="card-body">
            @if (count($templates) == 0)
                <h4 class="text-center">{{(__('setting.NoTemplatesAvailable'))}}.</h4>
            @else
                <table class="table table-striped" id="template_table">
                    <thead>
                        <tr>
                            ...
                            <th>{{(__('setting.Languages'))}}</th>
                            <th>{{((__('setting.TemplateType')))}}</th>
                            <th>{{(__('setting.IsActive'))}}</th>
