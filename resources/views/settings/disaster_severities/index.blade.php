...
@endsection
@section('pagetitle')
{{(__('setting.DisasterSeverities'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item active">{{(__('setting.DisasterSeverities'))}}</li>
@endsection
@section('stylesheets')
    ...
    <script type="text/javascript">
        function deleteConfirmation(id) {
            swal.fire({
                title: "Delete?",
                type: 'question',
                text: "Are you sure you want to delete this disaster severity?",
                type: "warning",
                showCancelButton: !0,
                ...
                if (e.value === true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ url('settings/disaster_severities/delete') }}/" + id,
                        data: {
                            _token: CSRF_TOKEN
                        },
                        dataType: 'JSON',
...
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">{{(__('setting.DisasterSeveritiesList'))}}</h3>
        </div>
        ...
        <div class="card-body">
            ...
            @else
                <table class="table table-striped" id="disaster_severity_table">
                    <thead>
                        <tr>
...
@section('javascripts')
    ...
    <script>
        $(document).ready(function() {
            var table = $('#disaster_severity_table').DataTable({
                paging: false,
                info: false,
                colReorder: true,
