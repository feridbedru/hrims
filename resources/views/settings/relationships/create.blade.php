@extends('layouts.app')
@section('pagetitle')
{{(__('setting.NewRelationship'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('relationships.relationship.index') }}">{{(__('setting.Relationships'))}}</a></li>
    <li class="breadcrumb-item active">{{(__('setting.New'))}}</li>
@endsection
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title mb-1">{{(__('setting.CreateNewRelationship'))}}</h3>
        </div>
        <div class="card-body">
...
@section('javascripts')
    ...
    <script>
        $(function() {
            $('#create_relationship_form').validate({
                rules: {
                    name: {
                        required: true,
                        minlength: 2,
