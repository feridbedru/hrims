@extends('layouts.app')
@section('pagetitle')
{{(__('employee.Edit Relationship'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('relationships.relationship.index') }}">{{(__('employee.Relationship'))}}</a></li>
    <li class="breadcrumb-item active">{{(__('setting.edit'))}}</li>
@endsection
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title mb-1">{{(__('employee.Edit Relationship'))}}</h3>
        </div>
        <div class="card-body">
...
@section('javascripts')
    ...
    <script>
        $(function() {
            $('#edit_relationship_form').validate({
                rules: {
                    name: {
                        required: true,
                        minlength: 2,
