@extends('layouts.app')
@section('pagetitle')
{{(__('employee.New Template Type'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('template_types.template_type.index') }}">{{(__('employee.Template Type'))}}</a></li>
    <li class="breadcrumb-item active">{{(__('setting.New'))}}</li>
@endsection
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title mb-1">{{(__('employee.Create New Template Type'))}}</h3>
        </div>
        <div class="card-body">
...
@section('javascripts')
    ...
    <script>
        $(function() {
            $('#create_template_type_form').validate({
                rules: {
                    name: {
                        required: true,
                        minlength: 2,
