...
@endsection
@section('pagetitle')
{{(__('setting.TemplateType'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item active">{{(__('setting.TemplateType'))}}</li>
@endsection
@section('stylesheets')
    ...
    <script type="text/javascript">
        function deleteConfirmation(id) {
            swal.fire({
                title: "Delete?",
                type: 'question',
                text: "Are you sure you want to delete this template type?",
                type: "warning",
                showCancelButton: !0,
                ...
                if (e.value === true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ url('settings/template_types/delete') }}/" + id,
                        data: {
                            _token: CSRF_TOKEN
                        },
                        dataType: 'JSON',
...
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">{{(__('employee.Template Types List'))}}</h3>
        </div>
        <div class="card-body">
            @if (count($templateTypes) == 0)
                <h4 class="text-center">{{(__('employee.No Template Types Available'))}}.</h4>
            @else
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    <button class="btn btn-danger remove-data"
                                        onclick="deleteConfirmation({{ $templateType->id }})">
    ...
    <a href="{{ route('template_types.template_type.create') }}" class="btn btn-success"
