@extends('layouts.app')
@section('pagetitle')
{{(__('setting.NewLicenseType'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('license_types.license_type.index') }}">{{(__('setting.LicenseTypes'))}}</a></li>
...
@section('javascripts')
    ...
    <script>
        $(function() {
            $('#create_license_type_form').validate({
                rules: {
                    name: {
                        required: true,
                        minlength: 2,
