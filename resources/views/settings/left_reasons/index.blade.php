    ...
    <script type="text/javascript">
        function deleteConfirmation(id) {
            swal.fire({
                title: "Delete?",
                type: 'question',
                text: "Are you sure you want to delete this left reason?",
                type: "warning",
                showCancelButton: !0,
                ...
                if (e.value === true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ url('settings/left_reasons/delete') }}/" + id,
                        data: {
                            _token: CSRF_TOKEN
                        },
                        dataType: 'JSON',
...
@section('content')
                        ...
                        @foreach ($leftReasons as $leftReason)
                            <tr>
                                ...
                                <td class="text-center">
                                    @permission('setting_LeaveReasons_Edit')
                                    <a href="{{ route('left_reasons.left_reason.edit', $leftReason->id) }}"
                                        class="btn btn-warning mr-4" title="Edit Left Reason">
                                        <span class="fa fa-edit text-white" aria-hidden="true"></span>
                                    </a>
                                    @endpermission
                                    @permission('setting_LeaveReasons_Delete')
                                    <button class="btn btn-danger remove-data"
                                        onclick="deleteConfirmation({{ $leftReason->id }})">
    ...
    @permission('setting_LeaveReasons_AddNew')
...
@section('javascripts')
    ...
    <script>
        $(document).ready(function() {
            var table = $('#left_reason_table').DataTable({
                paging: false,
                info: false,
                colReorder: true,
            ...
            $("#left_reason_table_filter").addClass("d-inline float-right");
            $("<hr>").insertBefore("#left_reason_table");
        });
