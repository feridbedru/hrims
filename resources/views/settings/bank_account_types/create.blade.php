@extends('layouts.app')
@section('pagetitle')
{{(__('setting.NewBankAccountType'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('bank_account_types.bank_account_type.index') }}">{{(__('setting.BankAccountType'))}}</a>
    </li>
...
@section('javascripts')
    ...
    <script>
        $(function() {
            $('#create_bank_account_type_form').validate({
                rules: {
                    name: {
                        required: true,
                        minlength: 2,
