@extends('layouts.app')
@section('pagetitle')
{{(__('employee.Edit Bank Account Type'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('bank_account_types.bank_account_type.index') }}">{{(__('employee.Bank Account Type'))}}</a>
    </li>
...
@section('javascripts')
    ...
    <script>
        $(function() {
            $('#edit_bank_account_type_form').validate({
                rules: {
                    name: {
                        required: true,
                        minlength: 2,
