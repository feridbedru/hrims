@extends('layouts.app')
@section('pagetitle')
{{(__('setting.NewMaritalStatus'))}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('settings.setting.index') }}">{{(__('setting.Setting'))}}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('marital_statuses.marital_status.index') }}">{{(__('setting.MaritalStatus'))}}</a></li>
    <li class="breadcrumb-item active">{{(__('setting.New'))}}</li>
@endsection
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title mb-1">{{(__('setting.CreateNewMaritalStatus'))}}</h3>
        </div>
        <div class="card-body">
...
@section('javascripts')
    ...
    <script>
        $(function() {
            $('#create_marital_status_form').validate({
                rules: {
                    name: {
                        required: true,
                        minlength: 2,
